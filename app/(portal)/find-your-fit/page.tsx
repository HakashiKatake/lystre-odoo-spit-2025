...
const OCCASIONS = [
    { value: "casual", label: "Casual Day", icon: Sun, description: "Everyday comfort" },
    { value: "party", label: "Party", icon: PartyPopper, description: "Night out vibes" },
    { value: "wedding", label: "Wedding", icon: Heart, description: "Celebrate in style" },
    { value: "formal", label: "Formal/Office", icon: Briefcase, description: "Professional look" },
    { value: "festive", label: "Festive/Diwali", icon: Flame, description: "Traditional charm" },
    { value: "date", label: "Date Night", icon: HeartHandshake, description: "Impress someone special" },
    { value: "outdoor", label: "Outdoor/Travel", icon: Mountain, description: "Adventure ready" },
    { value: "gym", label: "Gym/Sports", icon: Dumbbell, description: "Active lifestyle" },
];
...
export default function FindYourFitPage() {
    ...
    const generateRecommendations = () => {
        ...
        setTimeout(() => {
            ...
            if (selectedSize) {
                filtered = filtered.filter((p) => !p.sizes || p.sizes.length === 0 || p.sizes.includes(selectedSize));
            }
            ...
            if (selectedOccasion) {
                const occasionTypeMap: Record<string, string[]> = {
                    ...
                    wedding: ["kurta", "sarees", "formals", "lehenga"],
                    formal: ["formals", "shirt", "pant"],
                    festive: ["kurta", "sarees", "lehenga"],
                ...
                if (matchingTypes.length > 0) {
                    filtered = filtered.filter((p) => matchingTypes.some((t) => p.type?.toLowerCase().includes(t)));
                }
            }
            ...
            <div className="max-w-6xl mx-auto px-6 py-12">
                ...
                <div className="text-center mb-12">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#F5EBE0] border-2 border-[#2B1810] mb-4">
                        ...
                        <span className="text-sm font-bold text-[#2B1810] uppercase tracking-wider">AI Style Assistant</span>
                    </div>
                    ...
                    <p className="text-lg text-[#8B7355] max-w-2xl mx-auto">
                        Tell us about yourself and the occasion, and we&apos;ll recommend the best outfits for you
                    </p>
                </div>
                            ...
                            <div key={s} className="flex items-center">
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center border-2 border-[#2B1810] font-bold transition-colors ${
                                        step >= s ? "bg-[#8B7355] text-white" : "bg-white text-[#2B1810]"
                                    }`}
                                >
                                    {step > s ? <Check size={18} /> : s}
                                </div>
                                {s < 3 && <div className={`w-16 h-1 ${step > s ? "bg-[#8B7355]" : "bg-[#E5D4C1]"}`} />}
                            </div>
                        ))}
                    </div>
                )}
                        ...
                        >
                            ...
                            {step === 1 && (
                                <div className="text-center">
                                    <h2 className="text-2xl font-serif text-[#2B1810] mb-8">Who are you shopping for?</h2>
                                        ...
                                        {CATEGORIES.map((cat) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-xl font-serif text-[#2B1810]">{cat.label}</span>
                                                </button>
                                            );
                                        })}
                                    </div>
                            ...
                            {step === 2 && (
                                <div className="text-center">
                                    <h2 className="text-2xl font-serif text-[#2B1810] mb-8">What&apos;s the occasion?</h2>
                                        ...
                                        {OCCASIONS.map((occ) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-lg font-serif text-[#2B1810] block">{occ.label}</span>
                                                    <span className="text-sm text-[#8B7355]">{occ.description}</span>
                                                </button>
                                            );
                                        })}
                                    </div>
                            ...
                            {step === 3 && (
                                <div className="text-center">
                                    <h2 className="text-2xl font-serif text-[#2B1810] mb-8">What&apos;s your size?</h2>
                    ...
                    {showResults && (
                        ...
                        >
                            ...
                            <div className="flex flex-wrap items-center justify-center gap-4 mb-8">
                                <div className="px-4 py-2 bg-white border-2 border-[#2B1810] flex items-center gap-2">
                                    {(() => {
                                        ...
                                        const IconComponent = cat?.icon;
                                        return IconComponent ? <IconComponent size={20} className="text-[#8B7355]" /> : null;
                                    })()}
                                    <span className="font-bold text-[#2B1810] capitalize">{selectedCategory}</span>
                                </div>
                                <div className="px-4 py-2 bg-white border-2 border-[#2B1810] flex items-center gap-2">
                                    {(() => {
                                        const occ = OCCASIONS.find((o) => o.value === selectedOccasion);
                                        const IconComponent = occ?.icon;
                                        return IconComponent ? <IconComponent size={20} className="text-[#8B7355]" /> : null;
                                    })()}
                                    <span className="font-bold text-[#2B1810]">{OCCASIONS.find((o) => o.value === selectedOccasion)?.label}</span>
                                </div>
                            ...
                            <h2 className="text-3xl font-serif text-[#2B1810] text-center mb-8">
                                Your Perfect <span className="text-[#8B7355]">Recommendations</span>
                            </h2>
                            ...
                            {recommendations.length > 0 ? (
                                ...
                                <div className="text-center py-16 bg-white border-2 border-[#2B1810]">
                                    ...
                                    <h3 className="text-2xl font-serif text-[#2B1810] mb-2">No matches found</h3>
                                    <p className="text-[#8B7355] mb-6">
                                        We couldn&apos;t find products matching your criteria. Try different options!
                                    </p>
                            ...
                            <div className="text-center mt-12">
                                <p className="text-[#8B7355] mb-4">Want to explore more options?</p>
                                <Link href="/products">
